...
contract ResplitBranchAuthorityVault {
    ...
    function enableFinalRoute(bool enabled) external onlyManager {
        require(promotionReady, "route");
        finalRouteEnabled = enabled;
    }
    ...
    function activateBinding() external {
        if (finalRouteEnabled && promotionReady && primaryArmed) {
